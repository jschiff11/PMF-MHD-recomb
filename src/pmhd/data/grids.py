...
# Neutrino decoupling (T = 1 MeV): modes whose natural horizon-crossing
# redshift would be earlier than this are instead initialized at Z_NUDEC, so
# we can skip over the periods of neutrino diffusion and free-streaming
# damping (see paper, Sec. "Fluid regimes in the early Universe"). Applied
# directly to zcrossarr below so every consumer of zcrossarr picks this up
# automatically.
Z_NUDEC = 1.0e6 / (8.617333262e-5 * cons.T0) - 1.0
...
def generate_z_arrays(karr=None, lamarr=None):
    ...
    for kind in range(len(karr)):
        ...
        zcrossarr[kind] = min(opt.brentq(horizcross, 0, 1e11), Z_NUDEC)
